...
class ClientController extends Controller
{
    ...
    {
        $data = Client::with('Owner', 'Restriction')->findorfail($id);
    ...
    {
        $data = Client::with('Owner', 'Restriction')->orderBy('id', 'DESC');
        if ($Request->search) {
    ...
    public function search_reservations_action(Request $Request, $id)
    {
        ...
        $data = Reservation::with('Owner', 'Vehicle')->where('client', $id)->where('status', '!=', 'completed')->whereBetween('created_at', [$startDate, $endDate]);
        if ($Request->search) {
            $data = $data->search(urldecode($Request->search));
        }
        $data = $data->cursorPaginate(50);
    ...
    public function filter_reservations_action(Request $Request, $id)
    {
        ...
        $data = Reservation::with('Owner', 'Vehicle')->where('client', $id)->whereBetween('created_at', [$startDate, $endDate]);
        if ($Request->search) {
            $data = $data->search(urldecode($Request->search));
        }
        $data = $data->cursorPaginate(50);
    ...
    public function search_payments_action(Request $Request, $id)
    {
        ...
        $data = Reservation::with('Owner', 'Vehicle', 'Payment')->where('client', $id)->where('status', '!=', 'completed')->whereBetween('created_at', [$startDate, $endDate]);
        if ($Request->search) {
            $data = $data->search(urldecode($Request->search));
        }
    ...
    public function filter_payments_action(Request $Request, $id)
    {
        ...
        $data = Reservation::with('Owner', 'Vehicle', 'Payment')->where('client', $id)->whereBetween('created_at', [$startDate, $endDate]);
        if ($Request->search) {
            $data = $data->search(urldecode($Request->search));
        }
        $data = $data->cursorPaginate(50)->through(function ($carry) {
    ...
    public function filter_recoveries_action(Request $Request, $id)
    {
        ...
        $data = Reservation::with('Owner', 'Vehicle', 'Recovery')->where('client', $id)->whereBetween('created_at', [$startDate, $endDate]);
        if ($Request->search) {
            $data = $data->search(urldecode($Request->search));
        }
        $data = $data->cursorPaginate(50)->through(function ($carry) {
