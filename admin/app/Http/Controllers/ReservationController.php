...
class ReservationController extends Controller
{
    ...
    {
        $data = Reservation::with('Owner', 'Vehicle', 'Client', 'Agency')->where('status', '!=', 'completed')->orderBy('id', 'DESC');
        if ($Request->search) {
    ...
    {
        $data = Reservation::with('Owner', 'Vehicle', 'Client', 'Agency')->orderBy('id', 'DESC');
        if ($Request->search) {
            $data = $data->search(urldecode($Request->search));
        }
        $data = $data->cursorPaginate(50);
