...
@section('title', __('Preview agency') . ' #' . $data->id)

@section('meta')
    <meta name="routes" content='{!! json_encode([
        'filterReservations' => route('actions.agencies.reservations.search', $data->id),
        'entireReservations' => route('actions.agencies.reservations.filter', $data->id),
        'filterRecoveries' => route('actions.agencies.recoveries.search', $data->id),
        'entireRecoveries' => route('actions.agencies.recoveries.filter', $data->id),
        'filterPayments' => route('actions.agencies.payments.search', $data->id),
        'entirePayments' => route('actions.agencies.payments.filter', $data->id),
        ...
        'csrf' => csrf_token(),
        'chart' => route('actions.agencies.chart', $data->id),
    ]) !!}' />
            ...
            <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <ul class="lg:col-span-4 grid grid-rows-1 grid-cols-2 lg:grid-rows-2 lg:grid-cols-3 gap-6">
                    <li
                        ...
                        <svg class="block w-10 lg:w-12 aspect-square pointer-events-none text-blue-500" fill="currentcolor"
                    ...
                    <li
                        ...
                        <svg class="block w-10 lg:w-12 aspect-square pointer-events-none text-teal-500" fill="currentcolor"
                            viewBox="0 -960 960 960">
                            <path
...
@section('scripts')
    ...
    <script src="{{ asset('js/agency/scene.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
