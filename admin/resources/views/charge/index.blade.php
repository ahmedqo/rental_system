...
@section('content')
    <div class="p-6 bg-x-white rounded-x-thin shadow-x-core">
        <neo-datavisualizer print search filter download title="{{ __('Charges list') }}">
            {{-- <a slot="end" title="{{ __('Create') }}" href="{{ route('views.charges.store') }}"
...
@section('scripts')
    <script src="{{ asset('js/charge/index.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
