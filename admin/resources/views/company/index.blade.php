...
@section('title', __('Companies list'))

@section('meta')
    <meta name="routes" content='{!! json_encode([
        'search' => route('actions.companies.search'),
        'patch' => route('views.companies.patch', 'XXX'),
        'scene' => route('views.companies.scene', 'XXX'),
        'clear' => route('actions.companies.clear', 'XXX'),
        'base' => Core::route(),
        'csrf' => csrf_token(),
    ]) !!}' />
@endsection
...
@section('content')
    <div class="p-6 bg-x-white rounded-x-thin shadow-x-core">
        <neo-datavisualizer print search filter download title="{{ __('Companies list') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.companies.store') }}"
...
@section('scripts')
    <script src="{{ asset('js/company/index.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
