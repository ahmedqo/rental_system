...
@section('title', __('Preview company') . ' #' . $data->id)

@section('meta')
    <meta name="routes" content='{!! json_encode([
        'filterReservations' => route('actions.companies.reservations.search', $data->id),
        'entireReservations' => route('actions.companies.reservations.filter', $data->id),
        'filterRecoveries' => route('actions.companies.recoveries.search', $data->id),
        'entireRecoveries' => route('actions.companies.recoveries.filter', $data->id),
        'filterPayments' => route('actions.companies.payments.search', $data->id),
        'entirePayments' => route('actions.companies.payments.filter', $data->id),
        'entireCharges' => route('actions.companies.charges.search', $data->id),
        ...
        'csrf' => csrf_token(),
        'search' => route('actions.companies.popular', $data->id),
        'chart' => route('actions.companies.chart', $data->id),
    ]) !!}' />
...
@section('content')
    <div class="w-full items-start grid grid-rows-1 grid-cols-1 gap-6">
        @if ($data->status !== 'active' || $vals->ended)
            <ul class="w-full flex flex-col gap-1">
                @if ($data->status !== 'active')
                    <div class="bg-red-500 text-x-white p-4 text-base font-x-thin rounded-x-thin shadow-x-core">
                        {{ __('Company not active for the moment') }}
                    </div>
                @endif
                @if ($vals->ended)
                    <div class="bg-red-500 text-x-white p-4 text-base font-x-thin rounded-x-thin shadow-x-core">
                        {{ __('This company has :num pending reservations where the dropoff date has passed', ['num' => $vals->ended]) }}
                    </div>
                @endif
            </ul>
        @endif
            ...
            <div class="grid grid-rows-1 grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <ul class="lg:col-span-4 grid grid-rows-1 grid-cols-2 lg:grid-rows-2 lg:grid-cols-3 gap-6">
                    ...
                    <li
                        ...
                        <svg class="lock w-10 lg:w-12 aspect-square pointer-events-none text-violet-500" fill="currentcolor"
                            viewBox="0 -960 960 960">
                            <path
                        ...
                        <div class="flex flex-1 flex-col">
                            <h2 class="text-center lg:text-end text-sm lg:text-base text-x-black font-x-thin">
                                {{ __('Mileage') }}</h2>
                            <p class="text-center lg:text-end text-base lg:text-lg text-gray-800">
                                {{ $vals->period * $data->mileage_per_day }}
                                {{ __('Km') }}
                            </p>
                        </div>
                    </li>
        ...
        <div class="flex flex-col gap-2 p-6 bg-x-white rounded-x-thin shadow-x-core">
            ...
            <div class="rounded-x-thin bg-x-white p-4 relative border border-x-shade">
                <div
        ...
        <div class="flex flex-col p-6 bg-x-white rounded-x-thin shadow-x-core">
            <neo-datavisualizer id="data-popular" class="scene-table"
                title="{{ __('Popular vehicles') }}"></neo-datavisualizer>
        </div>
        <div class="flex flex-col gap-2 p-6 bg-x-white rounded-x-thin shadow-x-core">
            <neo-datavisualizer id="data-reservations" class="scene-table" search title=" {{ __('Reservations list') }}">
                <neo-switch slot="start" id="filter-reservations" active></neo-switch>
            </neo-datavisualizer>
        </div>
        <div class="flex flex-col gap-2 p-6 bg-x-white rounded-x-thin shadow-x-core">
            <neo-datavisualizer id="data-payments" class="scene-table" search title=" {{ __('Payments list') }}">
                <neo-switch slot="start" id="filter-payments" active></neo-switch>
            </neo-datavisualizer>
        </div>
        <div class="flex flex-col gap-2 p-6 bg-x-white rounded-x-thin shadow-x-core">
            <neo-datavisualizer id="data-recoveries" class="scene-table" search title=" {{ __('Recoveries list') }}">
                <neo-switch slot="start" id="filter-recoveries" active></neo-switch>
            </neo-datavisualizer>
        </div>
        <div class="flex flex-col gap-2 p-6 bg-x-white rounded-x-thin shadow-x-core">
            <neo-datavisualizer id="data-charges" class="scene-table" search
                title=" {{ __('Charges list') }}"></neo-datavisualizer>
        </div>
    </div>
@endsection
...
@section('scripts')
    ...
    <script src="{{ asset('js/company/scene.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
