...
@section('title', __('Restrictions list'))


@section('meta')
    <meta name="routes" content='{!! json_encode([
        'search' => route('actions.restrictions.search'),
        'patch' => route('views.restrictions.patch', 'XXX'),
        'clear' => route('actions.restrictions.clear', 'XXX'),
        'csrf' => csrf_token(),
    ]) !!}' />
@endsection
...
@section('content')
    <div class="bg-x-white rounded-x-thin shadow-x-core">
        <neo-datavisualizer print search filter download title="{{ __('Restrictions list') }}">
            {{-- <a slot="end" title="{{ __('Create') }}" href="{{ route('views.restrictions.store') }}"
...
@section('scripts')
    <script src="{{ asset('js/restriction/index.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
