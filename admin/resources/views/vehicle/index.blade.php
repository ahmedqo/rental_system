...
@section('title', __('Vehicles list'))


@section('meta')
    <meta name="routes" content='{!! json_encode([
        'search' => route('actions.vehicles.search'),
        'patch' => route('views.vehicles.patch', 'XXX'),
        'scene' => route('views.vehicles.scene', 'XXX'),
        'clear' => route('actions.vehicles.clear', 'XXX'),
        'csrf' => csrf_token(),
    ]) !!}' />
@endsection
...
@section('content')
    <div class="p-6 bg-x-white rounded-x-thin shadow-x-core">
        <neo-datavisualizer print search filter download title="{{ __('Vehicles list') }}">
            {{-- <a slot="end" title="{{ __('Create') }}" href="{{ route('views.vehicles.store') }}"
...
@section('scripts')
    <script src="{{ asset('js/vehicle/index.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
