...
@section('content')
    <div class="bg-x-white rounded-x-thin shadow-x-core">
        <neo-datavisualizer print search filter download header="{{ __('Agencies list') }}">
            <neo-tooltip slot="end" label="{{ __('Create') }}">
                <a href="{{ route('views.agencies.store') }}"
...
@section('scripts')
    <script src="{{ asset('js/agency/index.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
