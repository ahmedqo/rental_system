...
@section('meta')
    <meta name="routes" content='{!! json_encode([
        ...
        'patch' => route('views.clients.patch', 'XXX'),
        'scene' => route('views.clients.scene', 'XXX'),
        'clear' => route('actions.clients.clear', 'XXX'),
        'csrf' => csrf_token(),
    ]) !!}' />
@endsection
...
@section('content')
    <div class="p-6 bg-x-white rounded-x-thin shadow-x-core">
        <neo-datavisualizer print search filter download title="{{ __('Clients list') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.clients.store') }}"
...
@section('scripts')
    <script src="{{ asset('js/client/index.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
