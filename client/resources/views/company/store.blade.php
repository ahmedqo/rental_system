...
@section('content')
    <div class="w-full lg:w-2/3 mx-auto p-6">
        <h1 class="text-x-black font-x-huge text-2xl lg:text-4xl text-center mb-6">
            {{ __('Subscribe') }}
        </h1>
        <form validate action="{{ route('actions.companies.store') }}" method="POST" enctype="multipart/form-data"
            class="block w-full">
            @csrf
            <neo-tab-wrapper outlet="outlet-1" class="w-full flex flex-col gap-6">
                <div class="flex flex-wrap items-center justify-between gap-2 lg:gap-6 relative isolate">
                    ...
                    @foreach (['company', 'contact', 'representative'] as $key)
                        <neo-tab-trigger tabindex="0" slot="triggers" for="outlet-{{ $loop->index + 1 }}"
                            class="flex w-max p-2 lg:px-4 items-center justify-center text-sm lg:text-lg font-x-thin text-x-black bg-x-white rounded-x-thin outline-none hover:bg-x-acent hover:text-x-white focus:bg-x-acent focus:text-x-white">
                            <span>{{ ucfirst(__($key)) }}</span>
                ...
                <div class="p-6 bg-x-white rounded-x-thin shadow-x-core w-full flex flex-col gap-6">
                    <neo-tab-outlet name="outlet-1" class="grid grid-cols-1 grid-rows-1 gap-6">
                        <div class="flex flex-col gap-1">
                            ...
                            <neo-imagetransfer rules="required"
                                errors='{"required": "{{ __('The logo field is required') }}"}'
                                class="aspect-square lg:aspect-[16/5] p-2" placeholder="{{ __('Logo') }} (*)"
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <neo-textbox rules="required"
                                errors='{"required": "{{ __('The ice number field is required') }}"}'
                                placeholder="{{ __('ICE number') }} (*)" name="ice_number"
                                value="{{ old('ice_number') }}"></neo-textbox>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-x-black font-x-thin text-base">
                                {{ __('Decision number') }} (*)
                            ...
                            <neo-textbox rules="required"
                                errors='{"required": "{{ __('The decision number is required') }}"}'
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <neo-textbox rules="required"
                                ...
                                value="{{ old('mileage_per_day') }}"></neo-textbox>
                        </div>
                    </neo-tab-outlet>
                    <neo-tab-outlet name="outlet-2" class="grid grid-cols-1 grid-rows-1 gap-6">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <neo-select rules="required" errors='{"required": "{{ __('The city field is required') }}"}'
                                search placeholder="{{ __('City') }} (*)" name="city">
                                @foreach (Core::citiesList() as $city)
                                    <neo-select-item value="{{ $city }}"
                                        {{ $city == old('city') ? 'active' : '' }}>
                                        {{ ucfirst(__($city)) }}
                                    </neo-select-item>
                                @endforeach
                            </neo-select>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <neo-textbox rules="required"
                                errors='{"required": "{{ __('The zipcode field is required') }}"}' type="number"
                                placeholder="{{ __('Zopcode') }} (*)" name="zipcode"
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <neo-textbox rules="required"
                                errors='{"required": "{{ __('The address field is required') }}"}'
                    ...
                    <neo-tab-outlet name="outlet-3" class="grid grid-cols-1 grid-rows-1 gap-6">
                        <div class="flex flex-col gap-1">
                            ...
                            <neo-textbox rules="required"
                                errors='{"required": "{{ __('The first name field is required') }}"}'
                                placeholder="{{ __('First name') }} (*)" name="first_name"
                                value="{{ ucfirst(old('first_name') ?? '') }}"></neo-textbox>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-x-black font-x-thin text-base">
                                {{ __('Last name') }} (*)
                            ...
                            <neo-textbox rules="required"
                                errors='{"required": "{{ __('The last name field is required') }}"}'
                                placeholder="{{ __('Last name') }} (*)" name="last_name"
                                value="{{ ucfirst(old('last_name') ?? '') }}"></neo-textbox>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-x-black font-x-thin text-base">
                                {{ __('Email') }} (*)
                            ...
                            <neo-textbox rules="required|email"
                                errors='{"required": "{{ __('The email field is required') }}", "email": "{{ __('The email field must be a valid email') }}"}'
                                type="email" placeholder="{{ __('Email') }} (*)" name="representative_email"
                                value="{{ old('representative_email') }}"></neo-textbox>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="text-x-black font-x-thin text-base">
                                {{ __('Phone') }} (*)
                            ...
                            <neo-textbox rules="required|phone"
                                errors='{"required": "{{ __('The phone field is required') }}", "phone": "{{ __('The phone field must be a valid phone number') }}"}'
                                type="tel" placeholder="{{ __('Phone') }} (*)" name="representative_phone"
                                value="{{ old('representative_phone') }}"></neo-textbox>
                        </div>
                    </neo-tab-outlet>
