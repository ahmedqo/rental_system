...
@section('title', __('Reminders list'))
...
@section('content')
    <div class="bg-x-white rounded-x-thin shadow-x-core">
        <neo-datavisualizer print search filter download header="{{ __('Reminders list') }}">
            <neo-tooltip slot="end" label="{{ __('Create') }}">
                <a href="{{ route('views.reminders.store') }}" aria-label="create_page_link"
...
@section('scripts')
    <script src="{{ asset('js/reminder/index.min.js') }}?v={{ env('APP_VERSION') }}"></script>
@endsection
