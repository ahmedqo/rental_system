...
@section('content')
    <form validate action="{{ route('actions.reservations.patch', $data->id) }}" method="POST" class="w-full">
        @csrf
        @method('patch')
            ...
            <div class="w-full flex flex-col gap-6 p-6 bg-x-white rounded-x-thin shadow-x-core">
                <neo-tab-outlet name="outlet-1" class="grid grid-cols-1 grid-rows-1 gap-6">
                        ...
                        <neo-select rules="required" errors='{"required": "{{ __('The renter field is required') }}"}'
                            ...
                            @foreach (Core::reservationsList() as $renter)
                                <neo-select-item value="{{ $renter }}"
                                    {{ $renter == old('renter', $data->agency ? 'agency' : 'individual') ? 'active' : '' }}>
                                    {{ ucfirst(__($renter)) }}
                                </neo-select-item>
                            @endforeach
                        </neo-select>
                    ...
                    <div show-if="renter,individual" class="flex flex-col gap-1">
                        ...
                        <neo-autocomplete rules="custom_clash:client"
                            ...
                            name="secondary_client" value="{{ old('secondary_client', $data->secondary_client) }}"
                            query="{{ old('secondary_client_name', $data->secondary_client ? strtoupper($data->SClient->last_name) . ' ' . ucfirst($data->SClient->first_name) : null) }}">
                            <input type="hidden" name="secondary_client_name"
                                value="{{ old('secondary_client_name', $data->secondary_client ? strtoupper($data->SClient->last_name) . ' ' . ucfirst($data->SClient->first_name) : null) }}">
                        </neo-autocomplete>
                    </div>
                    <div show-if="renter,agency" class="flex flex-col gap-1">
                        ...
                        <neo-autocomplete rules="required_if:renter,agency"
                            ...
                            value="{{ old('agency', $data->agency) }}"
                            query="{{ old('agency_name', $data->agency ? ucfirst($data->Agency->name) : null) }}">
                            <input type="hidden" name="agency_name"
                                value="{{ old('agency_name', $data->agency ? ucfirst($data->Agency->name) : null) }}">
                        </neo-autocomplete>
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="text-x-black font-x-thin text-base">
                ...
                <neo-tab-outlet name="outlet-2" class="grid grid-cols-1 grid-rows-1 gap-6">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <neo-timepicker rules="required"
                            ...
                            value="{{ Carbon\Carbon::parse(old('pickup_date', $data->pickup_date))->format('H:i') ?? '#now' }}"></neo-timepicker>
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="text-x-black font-x-thin text-base">
                            {{ __('Pickup location') }}
                        ...
                        <neo-textbox placeholder="{{ __('Pickup location') }}" name="pickup_location"
                            value="{{ old('pickup_location', $data->pickup_location) }}"></neo-textbox>
                    </div>
                </neo-tab-outlet>
                <neo-tab-outlet name="outlet-3" class="grid grid-cols-1 grid-rows-1 gap-6">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <neo-timepicker rules="required"
                            ...
                            value="{{ Carbon\Carbon::parse(old('dropoff_date', $data->dropoff_date))->format('H:i') ?? '#now' }}"></neo-timepicker>
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="text-x-black font-x-thin text-base">
                            {{ __('Dropoff location') }}
                        ...
                        <neo-textbox placeholder="{{ __('Dropoff location') }}" name="dropoff_location"
                            value="{{ old('dropoff_location', $data->dropoff_location) }}"></neo-textbox>
                    </div>
                </neo-tab-outlet>
                <neo-tab-outlet name="outlet-4" class="grid grid-cols-1 grid-rows-1 gap-6">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <neo-textbox rules="required"
                            ...
                            value="{{ old('fuel_level', $data->fuel_level) }}"></neo-textbox>
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="text-x-black font-x-thin text-base">
                            {{ __('Mileage') }} (*)
                ...
                <neo-tab-outlet name="outlet-6" class="grid grid-cols-1 grid-rows-1 gap-6">
                    <input type="hidden" name="payment"
                        value='{{ old('payment', $data->Payment ? $data->Payment->logs : null) ?? '[]' }}'>
